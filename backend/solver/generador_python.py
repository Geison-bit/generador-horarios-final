...
class StressProgressCallback(cp_model.CpSolverSolutionCallback):
    ...
    def on_solution_callback(self):
        ...
        ahora = time.time()
        if (
            self.last_reported_at is not None
            and ahora - self.last_reported_at < self.print_interval_seconds
        ):
            return
        self.last_reported_at = ahora
        ...
        _emitir_evento_progreso(
            ...
            elapsed_seconds=ahora - self.start_time,
...
def generar_horario_cp(
    ...
    num_bloques = normalizar_entero((restricciones or {}).get("num_bloques")) or 8
    num_bloques = max(1, min(num_bloques, 12))
    ...
    reglas_ia = (restricciones or {}).get("reglas_ia", []) or []
    catalogo_cursos = (restricciones or {}).get("catalogo_cursos", []) or []
    nombres_curso = {
        normalizar_entero(curso.get("id")): normalizar_texto(curso.get("nombre"))
        for curso in catalogo_cursos
        if isinstance(curso, dict)
    }

    def _regla_aplica_al_curso(regla, curso_id):
        nombre = nombres_curso.get(normalizar_entero(curso_id), "")
        incluidos = {normalizar_texto(x) for x in (regla.get("cursos") or []) if str(x).strip()}
        excluidos = {normalizar_texto(x) for x in (regla.get("excepto_cursos") or []) if str(x).strip()}
        if incluidos and nombre not in incluidos:
            return False
        if excluidos and nombre in excluidos:
            return False
        return True

    limite_materia_dia = None
    limite_docente_grado_dia = None
    bloque_unico_docente_grado = None
    evitar_huecos_docente = None
    preferencias_franja_curso = []

    def _tipo_regla_efectivo(regla):
        tipo = regla.get("tipo")
        contenido = normalizar_texto(" ".join(str(regla.get(campo) or "") for campo in (
            "texto_original", "formalizacion", "ambiguedad"
        )))
        if tipo in {"limite_materia_dia", "no_soportada"}:
            if "docente" in contenido and ("grado" in contenido or "seccion" in contenido):
                if "maximo" in contenido or "sum(bloques_docente_grado_dia)" in contenido:
                    return "limite_docente_grado_dia"
        if tipo == "no_soportada":
            if ("mismo grado" in contenido or "misma seccion" in contenido) and any(
                frase in contenido for frase in ("unico periodo continuo", "no podra regresar", "regresar a ensenar")
            ):
                return "bloque_unico_docente_grado_dia"
            if regla.get("dureza") == "soft" and "docente" in contenido and any(
                palabra in contenido for palabra in ("hueco", "consecutiv", "compact")
            ):
                return "evitar_huecos_docente"
            if "carga semanal total" in contenido and "una hora" in contenido:
                return "excluir_carga_una_hora"
        return tipo

    for regla_ia in reglas_ia:
        if not regla_ia.get("activa", True):
            continue
        tipo_regla = _tipo_regla_efectivo(regla_ia)
        if tipo_regla == "limite_materia_dia":
            limite = normalizar_entero(regla_ia.get("maximo"))
            if 1 <= limite <= num_bloques:
                candidato = {"maximo": limite, "dureza": regla_ia.get("dureza", "hard")}
                if limite_materia_dia is None or limite < limite_materia_dia["maximo"]:
                    limite_materia_dia = candidato
                elif limite == limite_materia_dia["maximo"] and candidato["dureza"] == "hard":
                    limite_materia_dia["dureza"] = "hard"
        elif tipo_regla == "limite_docente_grado_dia":
            limite = normalizar_entero(regla_ia.get("maximo"))
            if 1 <= limite <= num_bloques:
                candidato = {"maximo": limite, "dureza": regla_ia.get("dureza", "hard")}
                if limite_docente_grado_dia is None or limite < limite_docente_grado_dia["maximo"]:
                    limite_docente_grado_dia = candidato
                elif limite == limite_docente_grado_dia["maximo"] and candidato["dureza"] == "hard":
                    limite_docente_grado_dia["dureza"] = "hard"
        elif tipo_regla == "bloque_unico_docente_grado_dia":
            dureza = regla_ia.get("dureza", "hard")
            if bloque_unico_docente_grado != "hard":
                bloque_unico_docente_grado = dureza
        elif tipo_regla == "evitar_huecos_docente":
            dureza = regla_ia.get("dureza", "soft")
            if evitar_huecos_docente != "hard":
                evitar_huecos_docente = dureza
        elif tipo_regla == "preferencia_franja_curso":
            # La interfaz y Supabase usan numeracion humana desde 1.
            bloques = sorted({
                normalizar_entero(bloque) - 1
                for bloque in (regla_ia.get("bloques_preferidos") or [])
                if 1 <= normalizar_entero(bloque) <= num_bloques
            })
            if bloques and (regla_ia.get("cursos") or regla_ia.get("excepto_cursos")):
                preferencias_franja_curso.append((regla_ia, set(bloques)))

    reglas_exclusion_una_hora = [
        regla for regla in reglas_ia
        if regla.get("activa", True) and _tipo_regla_efectivo(regla) == "excluir_carga_una_hora"
    ]
    if reglas_exclusion_una_hora:
        map_asignaciones = [
            req for req in map_asignaciones
            if req["horas"] != 1
            or not any(_regla_aplica_al_curso(regla, req["curso"]) for regla in reglas_exclusion_una_hora)
        ]
        total_horas_requeridas = sum(req["horas"] for req in map_asignaciones)
        ...
        for doc_str, disponibilidad_docente in disponibilidad_map.items():
            ...
            for dia_idx, dia_nom in enumerate(DIAS):
                ...
                for bloque in range(num_bloques):
                    ...
                    permitido = bool(disponibilidad_docente.get(key1) or disponibilidad_docente.get(key2))
                    if not permitido:
    ...
    def _obtener_patrones(req):
        encontrados_globales = []
        encontrados_especificos = []
        durezas_globales = []
        durezas_especificas = []
        for regla_ia in reglas_ia:
            if not regla_ia.get("activa", True) or regla_ia.get("tipo") != "division_horas":
                continue
            if normalizar_entero(regla_ia.get("total_horas")) != req["horas"]:
                continue
            if not _regla_aplica_al_curso(regla_ia, req["curso"]):
                continue
            raw_patrones = regla_ia.get("patrones") or []
            if not raw_patrones and regla_ia.get("patron"):
                raw_patrones = [regla_ia.get("patron")]
            destino = (
                encontrados_especificos
                if regla_ia.get("cursos") or regla_ia.get("excepto_cursos")
                else encontrados_globales
            )
            destino_durezas = (
                durezas_especificas
                if regla_ia.get("cursos") or regla_ia.get("excepto_cursos")
                else durezas_globales
            )
            destino_durezas.append(regla_ia.get("dureza", "hard"))
            for raw_patron in raw_patrones:
                try:
                    patron_ia = [int(x) for x in (raw_patron or [])]
                except Exception:
                    patron_ia = []
                if patron_ia and sum(patron_ia) == req["horas"] and patron_ia not in destino:
                    destino.append(patron_ia)
        # Una regla nombrada o con excepciones tiene prioridad sobre una regla
        # global antigua de la misma carga. Así 2+1 no se filtra a Arte o E. Física.
        if encontrados_especificos:
            return encontrados_especificos, ("hard" if "hard" in durezas_especificas else "soft")
        if encontrados_globales:
            return encontrados_globales, ("hard" if "hard" in durezas_globales else "soft")
        ...
        if isinstance(raw, str):
            ...
            return ([partes] if partes else []), "hard"
    ...
    # Las reglas suaves se expresan como variables de incumplimiento que el
    # objetivo intentará minimizar. Las reglas duras se agregan directamente.
    penalizaciones_reglas_suaves = []

    # D) Limite y bloque unico por docente, grupo y dia.
    # Permite combinar materias (por ejemplo 1h + 2h), pero obliga a que formen
    # un solo tramo continuo y que no excedan el maximo configurado.
    reqs_por_docente_grupo = {}
    for idx, req in enumerate(map_asignaciones):
        reqs_por_docente_grupo.setdefault((req["docente"], req["grupo"]), []).append(idx)

    if limite_docente_grado_dia is not None or bloque_unico_docente_grado:
        for (docente, grupo), indices in reqs_por_docente_grupo.items():
            for d in range(NUM_DIAS):
                ocupacion = []
                for b in range(num_bloques):
                    ocupado = model.NewBoolVar(f"ocupa_doc_grupo_{docente}_{grupo}_{d}_{b}")
                    model.Add(ocupado == sum(x[(idx, d, b)] for idx in indices))
                    ocupacion.append(ocupado)

                if limite_docente_grado_dia is not None:
                    maximo = limite_docente_grado_dia["maximo"]
                    if limite_docente_grado_dia["dureza"] == "soft":
                        exceso = model.NewIntVar(0, num_bloques, f"exceso_doc_grupo_{docente}_{grupo}_{d}")
                        model.Add(exceso >= sum(ocupacion) - maximo)
                        penalizaciones_reglas_suaves.append(exceso)
                    else:
                        model.Add(sum(ocupacion) <= maximo)

                if bloque_unico_docente_grado:
                    inicios = []
                    for b, ocupado in enumerate(ocupacion):
                        inicio = model.NewBoolVar(f"inicio_doc_grupo_{docente}_{grupo}_{d}_{b}")
                        if b == 0:
                            model.Add(inicio == ocupado)
                        else:
                            model.Add(inicio <= ocupado)
                            model.Add(inicio <= 1 - ocupacion[b - 1])
                            model.Add(inicio >= ocupado - ocupacion[b - 1])
                        inicios.append(inicio)
                    if bloque_unico_docente_grado == "soft":
                        exceso_inicios = model.NewIntVar(0, num_bloques - 1, f"exceso_tramos_{docente}_{grupo}_{d}")
                        model.Add(exceso_inicios >= sum(inicios) - 1)
                        penalizaciones_reglas_suaves.append(exceso_inicios)
                    else:
                        model.Add(sum(inicios) <= 1)

    # 4. Reglas aprobadas con IA: division de sesiones y limite diario.
    # Sin una regla IA no se impone ningun patron de division predeterminado.
    patrones_por_idx = {}
    for idx, req in enumerate(map_asignaciones):
        patrones_crudos, dureza_patron = _obtener_patrones(req)
        patrones = [p for p in patrones_crudos if p and sum(p) == req["horas"]]
        patrones_por_idx[idx] = patrones
        duraciones = sorted({k for patron in patrones for k in patron})
        cumple_patron = None
        if patrones and dureza_patron == "soft":
            cumple_patron = model.NewBoolVar(f"cumple_patron_{idx}")
            violacion_patron = model.NewBoolVar(f"viola_patron_{idx}")
            model.Add(cumple_patron + violacion_patron == 1)
            penalizaciones_reglas_suaves.append(violacion_patron)
        ...
        for d in range(NUM_DIAS):
            ...
            if limite_materia_dia is not None:
                maximo = limite_materia_dia["maximo"]
                if limite_materia_dia["dureza"] == "soft":
                    exceso = model.NewIntVar(0, num_bloques, f"exceso_materia_{idx}_{d}")
                    model.Add(exceso >= horas_dia[(idx, d)] - maximo)
                    penalizaciones_reglas_suaves.append(exceso)
                else:
                    model.Add(horas_dia[(idx, d)] <= maximo)

            if not patrones:
                continue

            tabla_duraciones = model.AddAllowedAssignments(
                [horas_dia[(idx, d)]],
                [[0]] + [[duracion] for duracion in duraciones],
            )
            if cumple_patron is not None:
                tabla_duraciones.OnlyEnforceIf(cumple_patron)
            for k in duraciones:
            ...
            # Cada sesion definida por una regla IA ocupa bloques consecutivos.
            starts = []
            for b in range(num_bloques):
                ...
                if b == 0:
                    ...
                    model.AddBoolOr([x[(idx, d, b)].Not(), x[(idx, d, b - 1)], es_inicio])
                    model.AddImplication(es_inicio, x[(idx, d, b)])
                    model.AddImplication(es_inicio, x[(idx, d, b - 1)].Not())
                starts.append(es_inicio)
            continuidad = model.Add(sum(starts) <= 1)
            if cumple_patron is not None:
                continuidad.OnlyEnforceIf(cumple_patron)

        if patrones:
            elecciones = [model.NewBoolVar(f"elige_patron_{idx}_{p}") for p in range(len(patrones))]
            if cumple_patron is None:
                model.AddExactlyOne(elecciones)
            else:
                model.Add(sum(elecciones) == cumple_patron)
            for k in duraciones:
                conteo_patron = model.Add(
                    sum(es_k_dia[(idx, d, k)] for d in range(NUM_DIAS))
                    == sum(Counter(patron).get(k, 0) * elecciones[p] for p, patron in enumerate(patrones))
                )
                if cumple_patron is not None:
                    conteo_patron.OnlyEnforceIf(cumple_patron)

    # Objetivo flexible compartido. Cada termino vale una unidad para que las
    # preferencias puedan negociarse sin invalidar ninguna regla hard.
    terminos_objetivo = list(penalizaciones_reglas_suaves)

    # Preferencia flexible: minimizar huecos internos en la jornada de cada docente.
    if evitar_huecos_docente and num_bloques >= 3:
        huecos = []
        for docente, indices in reqs_por_docente.items():
            for d in range(NUM_DIAS):
                ocupacion = []
                for b in range(num_bloques):
                    ocupado = model.NewBoolVar(f"ocupa_doc_{docente}_{d}_{b}")
                    model.Add(ocupado == sum(x[(idx, d, b)] for idx in indices))
                    ocupacion.append(ocupado)
                for b in range(1, num_bloques - 1):
                    hay_antes = model.NewBoolVar(f"doc_antes_{docente}_{d}_{b}")
                    hay_despues = model.NewBoolVar(f"doc_despues_{docente}_{d}_{b}")
                    model.AddMaxEquality(hay_antes, ocupacion[:b])
                    model.AddMaxEquality(hay_despues, ocupacion[b + 1:])
                    hueco = model.NewBoolVar(f"hueco_doc_{docente}_{d}_{b}")
                    model.Add(hueco <= hay_antes)
                    model.Add(hueco <= hay_despues)
                    model.Add(hueco <= 1 - ocupacion[b])
                    model.Add(hueco >= hay_antes + hay_despues - ocupacion[b] - 1)
                    if evitar_huecos_docente == "hard":
                        model.Add(hueco == 0)
                    else:
                        huecos.append(hueco)
        terminos_objetivo.extend(huecos)

    # Preferencia flexible: ubicar los cursos seleccionados en sus franjas.
    penalizaciones_franja = []
    for idx, req in enumerate(map_asignaciones):
        for regla, bloques_preferidos in preferencias_franja_curso:
            if not _regla_aplica_al_curso(regla, req["curso"]):
                continue
            for d in range(NUM_DIAS):
                for b in range(num_bloques):
                    if b not in bloques_preferidos:
                        if regla.get("dureza", "soft") == "hard":
                            model.Add(x[(idx, d, b)] == 0)
                        else:
                            penalizaciones_franja.append(x[(idx, d, b)])
    terminos_objetivo.extend(penalizaciones_franja)
    ...
    def _diagnosticar_datos():
        ...
        for grupo, indices in sorted(reqs_por_grupo.items()):
            ...
            sin_patron_indices = [idx for idx in indices if not _obtener_patrones(map_asignaciones[idx])[0]]
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        metricas["violaciones_preferencia_franja"] = sum(
            solver.Value(variable) for variable in penalizaciones_franja
        )
    else:
        metricas["violaciones_preferencia_franja"] = None
