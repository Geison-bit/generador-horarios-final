...
```powershell
python -m research.experiments.stress_runner
```
...
```text
research/results/stress_cp_sat.csv
research/results/stress_cp_sat_x30.json
research/results/logs/stress_x1.log
...
research/results/logs/stress_x30.log
```
...
```powershell
python -m research.experiments.stress_runner --no-time-limit
```
...
```powershell
python -m research.experiments.stress_runner --time-limit 60 --workers 8
```

Ejecutar solo algunas escalas:

```powershell
python -m research.experiments.stress_runner --multipliers 1 2 5
```

Cambiar la salida:

```powershell
python -m research.experiments.stress_runner --output research/results/experimento_01.csv
```
...
```powershell
python -m research.experiments.stress_runner --multipliers 30 --no-time-limit --progress
```

Ejecutar x30 con monitor y limite de una hora:

```powershell
python -m research.experiments.stress_runner --multipliers 30 --time-limit 3600 --progress
```
...
```powershell
python -m research.experiments.stress_runner --multipliers 30 --no-time-limit --progress-interval 60
```

Ver el log de progreso:

```powershell
type research\results\logs\stress_x30_progress.log
```

Ver el log interno completo de CP-SAT:

```powershell
type research\results\logs\stress_x30_solver.log
```
