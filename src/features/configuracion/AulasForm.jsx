...
import { Building2, Save, X, Pencil, Trash2, Loader2, Clock3 } from "lucide-react";
...
export default function AulasForm() {
  ...
  const registrarUltimaEdicion = async ({ operation, rowId, newValues = null }) => {
    const { data: authData } = await supabase.auth.getUser();
    const user = authData?.user;
    dispatch({
      type: "SET_ULTIMA_EDICION",
      payload: {
  ...
  useEffect(() => {
    const fetchUltima = async () => {
      ...
      // Conserva la actualización local cuando audit_logs no está disponible por RLS o configuración.
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (modoEdicion && aulaEditandoId) {
      ...
      await registrarUltimaEdicion({ operation: "update", rowId: aulaEditandoId, newValues: payload });
    } else {
      ...
      await registrarUltimaEdicion({ operation: "insert", rowId: data?.id, newValues: payload });
    }
  ...
  const eliminarAula = async (id) => {
    ...
    await registrarUltimaEdicion({ operation: "delete", rowId: id });
    await refrescarAulas();
  };
