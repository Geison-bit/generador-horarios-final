...
  const params = new URLSearchParams(location.search);
  ...
  const registrarUltimaEdicion = async ({ operation, rowId, newValues = null }) => {
    ...
    await logAudit({
      ...
      operation,
      rowId,
      newValues: { ...newValues, nivel: nivelURL },
    });
  };
...
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const reportError = (error, action) => {
      ...
      if (error.code === "23505") {
        alert(
          "No se pudo actualizar: ya existe un registro con los mismos datos."
        );
        return true;
      }
    ...
    const operation = docenteEditandoId ? "update" : "insert";
    ...
    if (docenteId) {
      ...
      await registrarUltimaEdicion({ operation, rowId: docenteId, newValues: payload });
    }
