...
import { cargarReglasIAActivas } from "../../services/reglasIAService";
...
const HorarioTable = () => {
  ...
  // --- CONTEXTO Y RUTA ---
  const { docentes, asignaciones, horasCursos, setHorarioGeneral } = useDocentes();
  const location = useLocation();
  const params = new URLSearchParams(location.search);
  const version = Number(params.get("version")) || 1;
  const configVersion = 1;
  const nivel = params.get("nivel") || "Secundaria";
  const storageKey = `historialHorarios:${nivel}:${configVersion}`;
  ...
  const docentesPorVersion = useMemo(
    () => (docentes || []).filter((d) => d.nivel === nivel && d.version_num === configVersion),
    [docentes, nivel]
  );
  ...
  const persistHistorial = async (schedules) => {
    ...
    try {
      await saveSharedScheduleGenerations(nivel, configVersion, normalized);
    } catch (error) {
  ...
  useEffect(() => {
    const cargarHistorial = async () => {
      ...
      try {
        const remoto = await listSharedScheduleGenerations(nivel, configVersion);
        if (remoto.length > 0) {
  ...
  // La disponibilidad sigue viniendo de la interfaz visual. Las restricciones
  // estructurales son invariantes internas del modelo y no son configurables.
  useEffect(() => {
    const cargarRestricciones = async () => {
      try {
        ...
        const [disponibilidadResp, reglasResp] = await Promise.all([
          supabase
          ...
          supabase
            .from("reglas_ia")
            .select("activa,parametros")
            .eq("nivel", nivel)
            .eq("tipo", "disponibilidad_docente"),
        ]);
        if (disponibilidadResp.error) throw disponibilidadResp.error;
        if (reglasResp.error) throw reglasResp.error;

        const docentesSinRestriccion = new Set(
          (reglasResp.data || [])
            .filter((regla) => regla.activa === false && regla.parametros?.docente_id != null)
            .map((regla) => String(regla.parametros.docente_id))
        );

        const base = {};
        (disponibilidadResp.data || []).forEach((r) => {
          const did = String(r.docente_id);
          if (docentesSinRestriccion.has(did)) return;
    ...
    cargarRestricciones();
  }, [nivel, configVersion, bloqueOneBased]);
  ...
  const validarEnHorario = (horario, docenteId, diaIndex, bloqueIndex, gradoIndexDestino) => {
    ...
    // La disponibilidad visual y el no-solape son datos estructurales, no reglas editables.
    if (nivel !== "Primaria") {
  ...
  const generarHorario = async () => {
    ...
    try {
      ...
      const disponibilidadParaEnviar = disponibilidadEfectiva || {};

      const reglasIAActivas = await cargarReglasIAActivas(nivel, configVersion);
      const payloadRestricciones = {
        disponibilidad: disponibilidadParaEnviar,
        reglas_ia: reglasIAActivas,
        catalogo_cursos: cursosDesdeDB.map((curso) => ({ id: curso.id, nombre: curso.nombre })),
        num_bloques: bloquesHorario.length || 8,
      };
      console.log(
      ...
      const horarioOptimizado = resultado.horario;
