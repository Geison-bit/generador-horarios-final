// Matriz visual de disponibilidad docente.
...
import { CalendarDays, History, Loader2, Save, Users, AlertCircle } from "lucide-react";
...
const RestriccionesForm = () => {
  ...
  const docenteDesdePanel = params.get("docente") || "";

  const versionSeleccionada = 1;
  ...
  useEffect(() => {
    if (
      docenteDesdePanel
      && docentes.some((docente) => String(docente.id) === docenteDesdePanel)
    ) {
      setDocenteSeleccionado(docenteDesdePanel);
    }
  }, [docenteDesdePanel, docentes]);
  ...
  const guardarRestricciones = async () => {
    ...
    try {
      ...
      setUltimaEdicion({
        actor_name: user?.user_metadata?.full_name || user?.email || "Usuario actual",
        actor_email: user?.email || null,
        created_at: new Date().toISOString(),
        operation: "update",
      });
      await logAudit({
        tableName: "restricciones_docente",
        operation: "update",
        rowId: docenteObj.id,
        newValues: {
          docente_id: docenteObj.id,
          docente: `${docenteObj.nombre} ${docenteObj.apellido || ""}`.trim(),
          bloques_disponibles: filasInsertar.length,
          nivel: nivelURL,
          version_num: versionSeleccionada,
        },
      });
        ...
        <div className="md:col-span-9">
          ...
          {!cargando && docentes.length === 0 && (
             <div className="flex items-center gap-2 mt-2 text-red-600 bg-red-50 p-2 rounded text-xs border border-red-100">
               ...
               <span>No se encontraron docentes <strong>activos</strong>. Vaya a "Registrar Docentes" para agregarlos.</span>
             </div>
          )}
        </div>
      </div>
