...
export default function SeccionesForm() {
  ...
  async function cargarVersiones() {
    ...
    const unique = data?.length
      ? [...new Set(data.map((r) => r.version_num))].sort((a, b) => a - b)
      : [1];
    setVersiones(unique);
  ...
  async function copiarDatosSeccion(origenId, destinoId) {
    ...
    const [horasResp, divResp, asigResp] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .select("curso_id, grado_id, total_horas, patron, nivel, version_num")
        .eq("seccion_id", origenId)
        .eq("nivel", nivel)
        .eq("version_num", versionNum),
      supabase
    ...
    if (divisiones.length) {
      const { error } = await supabase
        .from("horas_curso_grado_division")
        .upsert(divisiones, { onConflict: "curso_id,seccion_id,nivel,version_num" });
      if (error) throw error;
    }
    ...
    if (asignaciones.length) {
      const { error } = await supabase
        .from("asignaciones")
        .upsert(asignaciones, { onConflict: "curso_id,seccion_id,nivel,version_num" });
      if (error) throw error;
    }
  }
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      ...
      <div className="mt-4 mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <label htmlFor="secciones-version" className="text-xs text-slate-600">
            Version
          </label>
          <select
            id="secciones-version"
            value={versionNum}
